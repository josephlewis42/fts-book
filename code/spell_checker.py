...
class Checker:
	...
	def check_word(self, word, doc_dict={}):
		...
		try:
			...
			pass # ignore things that are numbers
			
		
		candidates = self.known([word]) or self.known(self.edits1(word)) or [word] #or self.known_edits2(word) or [word]
		try:
			nuword = max(candidates, key=doc_dict.get)
		except TypeError:
			nuword = list(candidates)[0]
